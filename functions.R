...
func_plot_counts_percents <- function(x, colour_fill, colour_txt, plot_title, plot_subtitle, plot_x) {
  ...
  plot_bar <- ggplot(data = data.frame(x), mapping = aes(x=x)) +
    ...
    geom_text(mapping = aes(y = (..count..),
              ...
              stat="bin", 
              colour = colour_txt) +
    labs(title = plot_title, subtitle = plot_subtitle, x = plot_x, y = "Count") +
