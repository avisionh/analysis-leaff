...
txt_rating <- data_motivation %>% 
  filter(Value > threshold) %>% 
  select(Field) %>% 
  pull()

txt_rating_count <- data_motivation %>% 
  distinct(Field) %>% 
  nrow()

if (txt_rating_count > 6) {
  txt_rating_range <- "diverse and far-reaching"
} else {
  txt_rating_range <- "concentrated"
}
```
...
```
+ In **Figure 7**, the main reasons most people attended were `r toString(txt_rating)`. ^[*Main reasons* being defined as those reasons which have received more than `r toString(threshold)` responses in the post-film survey.] 
...
+ **Figure 7** also demonstrates how `r toString(txt_rating_range)` the reasons for why people attend the festival are, thereby illustrating that the festival caters for a number of preferences, tastes and interests. In this regard, LEAFF 2017 was able to engage with attendees on many levels.
...
kable(table_future, caption = "_Likelihood of audience returning for future LEAFF editions based on their experience of LEAFF 2017_")
```
