...
```{r (HIDE) Section 1.1, include = FALSE, echo = FALSE}
...
txt_film_titles <- data_activity_fund %>% 
  ...
  pull()
txt_film_titles <- paste0(txt_film_titles[1:4], sep = ", ") %>% 
...
txt_film_screenings <- data_activity_fund %>% 
  distinct(VenueName) %>% 
  pull()
txt_film_screenings <- paste0(txt_film_screenings[1:3], sep = ", ") %>% 
...
txt_event <- data_activity_fund %>% 
  ...
  pull()
txt_event <- paste0(txt_event[1:length(txt_event)-1], sep = ", ") %>% 
