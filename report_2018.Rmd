...
retention_table <- data_retention %>% 
  select(Choice, Response) %>% 
...
txt_retention_new <- data_retention %>% 
...
func_plot_pie(x = data_retention, col_counts = "Percent", col_category = "Choice_New", 
              plot_title = "Audience Retention", factor_levels = lvls)
```
+ In **Figure 3**, we have `r toString(paste0(txt_retention_new, "%"))` of respondents saying this is their first time coming to the festival. 

+ Of the remaining `r toString(paste0(1 - txt_retention_new), "%")` of respondents who have returned from previous festivals, we have the following breakdown:
